...
import {
  BullMqUnitTestQueue,
  BullMqUnitTestQueueRegistry,
  createUnitTestBullQueue,
  unitTestQueueName,
} from './unit-test-queue.js';
...
describe('unitTestQueueName', () => {
  ...
  it('never contains a colon — BullMQ rejects queue names with one', () => {
    expect(unitTestQueueName('default')).not.toContain(':');
  });
});

describe('BullMqUnitTestQueueRegistry', () => {
  it('gives two different workerIds two distinct queue instances', () => {
    const registry = new BullMqUnitTestQueueRegistry(unreachableConnection());
    ...
    expect(laptopQueue).toBeInstanceOf(BullMqUnitTestQueue);
  });
