...
import { downloadReport, useGenerateReport, useReports } from '../api/hooks.js';

const FORMATS: ReportFormat[] = ['json', 'sarif', 'html', 'pdf'];

export function ReportActions({ scanId }: { scanId: string }) {
  const reportsQuery = useReports(scanId);
  const generate = useGenerateReport(scanId);
